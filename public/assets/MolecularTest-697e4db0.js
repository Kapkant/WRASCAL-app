import{m as D}from"./mockData-d4cc2636.js";import{d as ee,x as p,$ as H,A as le,o as g,c as S,w as e,a as l,Q as P,P as n,aN as te,N as r,T as V,aq as I,ar as j,aj as Z,aJ as A,O as L,U as oe,aO as ae,S as ne}from"./index-a6c0eccc.js";import{V as f,d as v,c as x}from"./VCard-a2da4aa9.js";import{a as T,V as re}from"./VCol-57be9264.js";const se=r("h2",null,"🧪 Molecular Visualization Test",-1),de=r("p",{class:"text-subtitle-1"},"Interactive molecular structure viewer",-1),ie=r("strong",null,"Name:",-1),ue=r("strong",null,"Formula:",-1),ce=r("strong",null,"SMILES:",-1),me=r("strong",null,"Description:",-1),pe={key:1,class:"text-grey"},ge={key:1,class:"text-grey"},be=ee({__name:"MolecularTest",setup(fe){const h=p(null),t=p(null),M=p(!1),w=p(!1),$=p(!1),k=p(!1),K=H(()=>D.map(a=>({title:`${a.name} (${a.molecularFormula})`,value:a.id}))),B=H(()=>`height: 300px; background: ${ne().global.current.value.dark?"gray":"transparent"}; border: 1px solid #ccc; border-radius: 4px;`),F=a=>{const o=D.find(d=>d.id===a);o&&(t.value=o.molData,console.log("Selected molecule:",t.value)),z()},z=()=>{M.value=!1,w.value=!1;const a=document.getElementById("mol2D"),o=document.getElementById("mol3D");a&&(a.innerHTML=""),o&&(o.innerHTML="")},O=async()=>{if(!t.value){console.log("No molecule selected");return}console.log("Loading 2D molecule:",t.value),$.value=!0;try{await ae("https://unpkg.com/@rdkit/rdkit/dist/RDKit_minimal.js");const a=await window.initRDKitModule(),o=t.value.drawCode,d=a.get_mol(o),i=d.get_molblock(),y=d.get_svg(500,400),m=document.getElementById("mol2D");m&&(m.innerHTML=`
      ...
      `,M.value=!0)}finally{$.value=!1}},q=async()=>{if(!t.value){console.log("No molecule selected");return}console.log("Loading 3D molecule:",t.value),k.value=!0;try{const a=t.value.drawCode;let o="",d=6;const i=60,y=150,m=100;for(let s=0;s<6;s++){const u=s*Math.PI*2/6,b=y+Math.cos(u)*i,_=m+Math.sin(u)*i,c=Math.sin(u*3)*20;o+=`
        <div style="
          position: absolute;
          left: ${b}px;
          top: ${_}px;
      ...
      `}let N="";for(let s=0;s<6;s++){const u=s*Math.PI*2/6,b=(s+1)*Math.PI*2/6,_=y+Math.cos(u)*i,c=m+Math.sin(u)*i,C=y+Math.cos(b)*i,E=m+Math.sin(b)*i,U=Math.sin(u*3)*20,X=Math.sin(b*3)*20,Y=(_+C)/2,J=(c+E)/2,Q=(U+X)/2,G=Math.sqrt((C-_)**2+(E-c)**2),W=Math.atan2(E-c,C-_)*180/Math.PI;N+=`
        <div style="
          position: absolute;
          left: ${Y}px;
          top: ${J}px;
          ...
          height: 4px;
          background: #666;
          transform: translateZ(${Q}px) rotate(${W}deg);
